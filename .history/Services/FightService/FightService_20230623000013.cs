...
public class FightService : IFightService
{
        ...
        try
        {
            ...
            while (!defeated)
            {
                foreach (var attacker in charaters)
                {
                    ...
                    response.Data.Log.Add($"{attacker.Name} attack {opponent.Name} using {attackUsed}");
    ...
    {
        var charaters = await context.Charaters.Where(c => c.Fight > 0).OrderByDescending(c => c.Victories)
    }
}