...
public class CharaterService : ICharaterService
{
    ...
    private int GetUserId() => int.Parse(httpContextAccessor.HttpContext.User.FindFirstValue(ClaimTypes))
    public async Task<ServiceResponse<List<GetCharaterDto>>> AddCharater(AddCharaterDto newCharater)
    {
        ...
        serviceResponse.Data = await context.Charaters.Select(c => mapper.Map<GetCharaterDto>(c)).ToListAsync();
    ...
    public async Task<ServiceResponse<List<GetCharaterDto>>> DeleteCharater(int id)
    {
        ...
        try
        {
            var charater = await context.Charaters.FirstOrDefaultAsync(c => c.Id == id);
            if (charater is null)
    ...
    public async Task<ServiceResponse<List<GetCharaterDto>>> GetAllCharaters(int userId)
    {
        var serviceResponse = new ServiceResponse<List<GetCharaterDto>>();
        var dbCharaters = await context.Charaters.Where(c => c.User!.Id == userId).ToListAsync();
    ...
    public async Task<ServiceResponse<GetCharaterDto>> GetCharaterById(int id)
    {
        ...
        var dbCharater = await context.Charaters.FirstOrDefaultAsync(c => c.Id == id);
    ...
    public async Task<ServiceResponse<GetCharaterDto>> UpdateCharater(UpdateCharaterDto updateCharater)
    {
        ...
        try
        {
            var charater = await context.Charaters.FirstOrDefaultAsync(c => c.Id == updateCharater.Id);
            if (charater is null)
